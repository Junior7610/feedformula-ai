...
MODEL = "gpt-image-2"
QUALITY = "high"
API_IMAGE_URL = "https://build.lewisnote.com/v1/images/generations"
...
ImageSize = Literal["1024x1024", "1280x720", "720x1280", "1536x1024", "1024x1536"]
...
def _request_payload(prompt: str, taille: ImageSize) -> Dict[str, Any]:
    return {
        "model": MODEL,
        "prompt": prompt,
        "size": taille,
...
def _generate_with_requests(prompt: str, taille: ImageSize) -> Optional[bytes]:
    ...
    response = requests.post(
        API_IMAGE_URL,
        headers=headers,
        json=_request_payload(prompt, taille),
        timeout=180,
    )
    if response.status_code != 200:
        print(f"⚠️ HTTP {response.status_code}: {response.text[:600]}")
        return None
...
def generer_image(
    nom_fichier: str, prompt: str, taille: ImageSize = "1536x1024"
) -> bool:
...
JOBS: List[ImageJob] = [
    ...
    ImageJob("splash_screen_v2.png", PROMPTS["splash_screen_v2.png"], "1024x1536"),
    ImageJob(
        "icones_animaux_production.png",
